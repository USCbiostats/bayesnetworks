...
{   // accumulate overall loglikelihood for the graph
  ...
  double loglike=0;
  int p = ChangedNode;
  if (all) for (p=0; p<P; p++) loglike += score(SY, SYY, SXY, sumX, sumXX, N, p, Npar, par, MaxPar, X, lnLR);
  else    loglike += score(SY, SYY, SXY, sumX, sumXX, N, p, Npar, par, MaxPar, X, lnLR);
  return (loglike);
}
